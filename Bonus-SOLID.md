...
Ao precisarmos mudar como os produtos são salvos no banco,
altera-se apenas o `ProdutoService` — sem tocar nos controllers, DTOs ou models.
Se a regra de negócio do estoque mudar, altera-se apenas o `PedidoService`.
...
Durante o desenvolvimento foi adicionado o PATCH aos produtos e
pedidos, criando novos métodos, além dos DTOs, sem modificar o comportamento já existente
...
Qualquer Controller pode substituir outro no pipeline sem
...
O `ProdutoPatchDTO` tem todos os campos **nullable** (`string?`,
...
A inversão de dependência foi o que tornou os testes unitários
